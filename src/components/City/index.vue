<template>
    <div class="city_body">
        ...
        <div class="city_index">
            <ul>
                <li v-for="(v,i) in cityList" :key="i" @touchstart="handleToIndex(i)">{{v.index}}</li>
            </ul>
        </div>
    </div>
</template>
    ...
    methods:{
        formatCityList(cities){
            ...
            for(var i=0;i<cities.length;i++){
                ...
                if(toCom(firstLetter)){//如果是true(cityList里没这个index 那么创建)
            ...
            function toCom(firstLetter){
                for(var i=0;i<cityList.length;i++){
                    if(cityList[i].index===firstLetter){//如果cityList集合中有和首字母相等的 返回false 触发else
                        return false
                    }
                }
                return true//返回true 直接触发 push到数组中
            }
            return {
                cityList,hotList
            }
